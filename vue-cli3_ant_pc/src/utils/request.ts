...
_AXIOS.interceptors.request.use((config) => {
  // 设置请求的次数
  if (OBJ[config.url as any] === undefined) {
    OBJ[config.url as any] = 0;
  }
...
_AXIOS.interceptors.response.use(
  (response) => {
    // 清空保存的地址
    delete OBJ[response.config.url as any];
    let data = response.data;
    /**
     * 判断是否成功
     */
    if (data.code != '00000') {
      message.error(data.msg || '本次请求后台出错了',3);
    }
    // 对响应数据做点什么
    ...
    if (error.message && error.message.toLocaleLowerCase().indexOf('timeout') > -1) {
      ...
      message.info('请求超时,您可以刷新页面重新请求',3);
      ...
      switch (error.response.status) {
        //处理后台响应的错误
        case 401:
          message.info('请登录后,在操作',3);
          router.replace({
            path: '/login'
          });
          return Promise.reject(error);
        case 404:
          message.error('接口不存在，请联系管理员',3);
          return Promise.reject(error);
        case 500:
          message.error('服务器出错啦，请求联系管理员',3);
          return Promise.reject(error);
        default:
